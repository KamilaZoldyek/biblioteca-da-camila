# Hello! 👋

Este é um projetinho que fiz como trabalho de conclusão de curso do programa LevelUp da Rethink.

## ✨ Qual é o propósito?

A Biblioteca da Camila é exatamente o que o nome diz: Uma biblioteca. Da Camila.
A ideia é eu poder catalogar meus livros impressos (principalmente meus mangás) e organizá-los. Assim, eu não preciso anotar quais volumes faltam para eu completar uma coleção quando for em um evento comprar mais :D


## ✨ Funcionalidades

(Esse bloco eu copiei do chatgpt e editei porque o coitado alucinou bonito.)

- 📖 Cadastro manual de livros (título, autor, editora, etc.)
- 📷 Escaneamento de código de barras (ISBN)
- 🔍 Pesquisa rápida por título e autor
- 🗂️ Organização por coleções/séries
- 📝 Edição de metadados que vem da API Google Books
- ☁️ Sincronização com banco remoto
- 🔔 Lembrete de leitura passivo-agressivo estilo Duolingo


## 📷 Prints

<img src="https://wrxchwepnruhjnsziquz.supabase.co/storage/v1/object/public/book-covers/github/login.png" width="300" />
<img src="https://wrxchwepnruhjnsziquz.supabase.co/storage/v1/object/public/book-covers/github/home.png" width="300" />
<img src="https://wrxchwepnruhjnsziquz.supabase.co/storage/v1/object/public/book-covers/github/book.png" width="300" />
<img src="https://wrxchwepnruhjnsziquz.supabase.co/storage/v1/object/public/book-covers/github/configs.png" width="300" />
<img src="https://wrxchwepnruhjnsziquz.supabase.co/storage/v1/object/public/book-covers/github/notification.png" width="300" />


## 🚀 Próximos Passos

- Guardar os dados offline
- Correção de bugs
- Mais filtros
- Inserir tags
- Deixar menos pessoal
- Edição de senha/recuperação de senha
- Edição de Coleções
- Publicação na loja


## 🛠️ Tecnologias Utilizadas

[Criei um Figma para o app](https://www.figma.com/design/Lp9v67gUnHqitixJn0kWxW/Biblioteca-da-Camila?m=auto&t=lTpPxNPjeO8vZGZf-1)

Essa lista eu mandei o chatgpt fazer:

### 📱 Core do App
![React Native](https://img.shields.io/badge/React%20Native-0.79.5-61dafb?logo=react&logoColor=white)
![Expo](https://img.shields.io/badge/Expo-53.0.20-000000?logo=expo)
![Expo Router](https://img.shields.io/badge/Expo%20Router-5.1.6-blueviolet)
![TypeScript](https://img.shields.io/badge/TypeScript-5.8.3-3178C6?logo=typescript)
![React](https://img.shields.io/badge/React-19.0.0-61dafb?logo=react)

---

### 🧭 Navegação
![React Navigation](https://img.shields.io/badge/React%20Navigation-7.x-ff7f50?logo=react)
![Screens](https://img.shields.io/badge/react--native--screens-4.11.1-ffcc00)
![Safe Area Context](https://img.shields.io/badge/Safe%20Area%20Context-5.4.0-ffcc00)

---

### 🧩 Interface e Layout
![React Native Paper](https://img.shields.io/badge/React%20Native%20Paper-5.14.5-6200ea)
![Super Grid](https://img.shields.io/badge/Super%20Grid-6.0.2-teal)

---

### 📷 Mídia e Imagens
![Expo Camera](https://img.shields.io/badge/Expo%20Camera-16.1.11-4caf50)
![Image Picker](https://img.shields.io/badge/Expo%20Image%20Picker-16.1.4-4caf50)
![Expo Image](https://img.shields.io/badge/Expo%20Image-2.4.0-009688)
![React Native SVG](https://img.shields.io/badge/react--native--svg-15.11.2-ff9800)

---

### ☁️ Backend e Integrações
![Supabase](https://img.shields.io/badge/Supabase-2.57.4-3fcf8e?logo=supabase&logoColor=white)
![Firebase](https://img.shields.io/badge/Firebase-12.4.0-ffca28?logo=firebase)
![Axios](https://img.shields.io/badge/Axios-1.12.2-5a29e4)

---

### 🔔 Notificações e Dispositivos
![Expo Notifications](https://img.shields.io/badge/Notifications-0.31.4-03a9f4)
![Expo Device](https://img.shields.io/badge/Device-7.1.4-03a9f4)
![Expo Constants](https://img.shields.io/badge/Constants-17.1.7-03a9f4)
![Expo Linking](https://img.shields.io/badge/Linking-7.1.7-03a9f4)
![Expo Web Browser](https://img.shields.io/badge/Web%20Browser-14.2.0-03a9f4)

---

### 🧠 Formulários e Estado
![React Hook Form](https://img.shields.io/badge/React%20Hook%20Form-7.62.0-ff4081)

---

### 💾 Armazenamento e Dados
![Async Storage](https://img.shields.io/badge/Async%20Storage-2.1.2-8bc34a)

---

### 🧰 Ferramentas e Dev
![ESLint](https://img.shields.io/badge/ESLint-9.36.0-4b32c3?logo=eslint)
![Expo CLI](https://img.shields.io/badge/Expo%20CLI-16.19.3-000000?logo=expo)
![Supabase CLI](https://img.shields.io/badge/Supabase%20CLI-2.51.0-3fcf8e?logo=supabase)
![Type Definitions](https://img.shields.io/badge/@types/react-19.0.10-blue)
![Babel](https://img.shields.io/badge/Babel-7.25.2-f9dc3e?logo=babel)
![SVG Transformer](https://img.shields.io/badge/SVG%20Transformer-1.5.1-ff9800)

---

### 🍞 Outros
![Keyboard Aware ScrollView](https://img.shields.io/badge/Keyboard%20Aware%20ScrollView-0.9.5-009688)
![Reanimated](https://img.shields.io/badge/Reanimated-3.17.4-795548)


## ✨ Minhas redes sociais

@kamila_zoldyek no Instagram


